...
from mkprobes.codebook.finalconstruct import (
    construct_encoding,
    construct_idt,
)
...
def mock_finalconstruct_dependencies(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("mkprobes.codebook.finalconstruct.READOUTS", MOCKED_READOUTS)
    monkeypatch.setattr("mkprobes.codebook.finalconstruct.rc", mock_rc)
    monkeypatch.setattr("mkprobes.codebook.finalconstruct.rotate", mock_rotate)
    monkeypatch.setattr(
        "mkprobes.codebook.finalconstruct.test_splint_padlock", mock_test_splint_padlock
    )
...
def test_construct_idt_homopolymer_error(
    ...
    original_rotate = __import__("mkprobes.codebook.finalconstruct", fromlist=["rotate"]).rotate
    monkeypatch = pytest.MonkeyPatch()
    monkeypatch.setattr("mkprobes.codebook.finalconstruct.rotate", lambda seq, n: seq)

    with pytest.raises(ValueError, match="Homopolymers"):
        construct_idt(challenging_idt_input, [7])  # Use READOUTS[7]

    monkeypatch.setattr("mkprobes.codebook.finalconstruct.rotate", original_rotate)  # Restore
...
def test_construct_encoding_single_idx_delegates_to_construct_idt(
    ...
    mocked_idt = mocker.patch("mkprobes.codebook.finalconstruct.construct_idt")
