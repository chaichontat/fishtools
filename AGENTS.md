...
- DO NOT use shell here-docs with `conda run` in this environment, especially `conda run -n seq python - <<'PY'`. This combination is unreliable under the agent wrapper: it can exit `0` while silently dropping stdout/stderr and sometimes not executing the Python body at all. That wastes time and tokens. Use `conda run -n seq python -c "..."` for short commands, or write a real temporary script file and execute that file with `conda run`.
- Treat missing side effects after a `conda run` here-doc as operator error, not a transient glitch. If you expected a file, plot, or printed output and it is absent, stop repeating the same pattern and switch immediately to `python -c` or a script file.
- NEVER postprocess plots or other rendered images to change titles, labels, legends, or annotations. Make the source plotting code produce the correct output and rerender from code instead.
