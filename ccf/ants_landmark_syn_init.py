...
def _parse_int_tuple(value: str) -> tuple[int, ...]:
    ...
    if not parts:
        raise click.BadParameter("Expected a comma-separated list of integers.")
    try:
        return tuple(int(p) for p in parts)
    except ValueError as exc:
        raise click.BadParameter(f"Invalid int list: {value!r}.") from exc


@click.command("ants-landmark-syn-init")
@click.argument(
    "workspace",
    type=click.Path(
        exists=True,
...
@click.option(
    ...
    show_default=True,
    help="Output subdirectory name under <workspace>/analysis/output/ccf-transforms/<roi>/.",
)
@click.option(
    "--debug/--no-debug",
    default=False,
...
@click.option(
    ...
    type=float,
    default=None,
    help="Override p1_threshold.json (raw-intensity threshold used to build the moving mask).",
)
@click.option(
    "--landmark-linear-transform-type",
    type=click.Choice(["similarity", "affine"], case_sensitive=False),
    default="affine",
    show_default=True,
    help="Linear init fit to landmarks (fixed->moving) before SyN.",
)
@click.option(
    "--syn-type-of-transform",
    default="SyNOnly",
    show_default=True,
    help="ANTsPy registration preset (must honor syn_metric when using MI).",
)
@click.option(
    "--syn-metric",
    type=click.Choice(["mattes", "cc"], case_sensitive=False),
    default="mattes",
    show_default=True,
    help="SyN deformable metric.",
)
@click.option(
    "--syn-metric-param",
    type=int,
    default=32,
    show_default=True,
    help="SyN metric parameter: Mattes=nbins, CC=radius.",
)
@click.option(
    "--syn-reg-iterations",
    default="300,150,70,30",
    show_default=True,
    help="SyN iterations per level as comma-separated ints.",
)
@click.option(
    "--syn-grad-step",
    type=float,
    default=0.15,
    show_default=True,
    help="SyN gradient step (regularization knob).",
)
@click.option(
    "--syn-flow-sigma",
    type=float,
...
@click.option(
    ...
    default=1,
    show_default=True,
    help="SyN total sigma.",
)
@click.option(
    "--moving-presmooth-sigma-um",
    type=float,
    default=20.0,
    show_default=True,
    help="Pre-smooth moving crop (um) before normalization/features.",
)
@click.option(
    "--fixed-edge-guard-um",
    type=float,
    default=0.0,
    show_default=True,
    help="Erode fixed mask by this many um before registration.",
)
@click.option(
    "--moving-edge-guard-um",
    type=float,
    default=0.0,
    show_default=True,
    help="Erode moving mask by this many um before registration.",
)
@click.option(
    "--use-affine-refine/--no-use-affine-refine",
    default=False,
    show_default=True,
    help="Run an image-driven affine/Rigid refine after landmark init and before SyN.",
)
@click.option(
    "--use-n4/--no-use-n4",
    default=False,
...
@click.option(
    "--use-feature-images/--no-use-feature-images",
    default=False,
    show_default=True,
    help="Use gradient-magnitude feature images in addition to intensity.",
)
@click.option(
    "--use-mask-distance-metric/--no-use-mask-distance-metric",
    default=True,
    show_default=True,
    help="Inject mask signed-distance map as an extra metric channel.",
)
@click.option(
    "--use-landmark-heatmap-metric/--no-use-landmark-heatmap-metric",
    default=True,
    show_default=True,
...
@click.option(
    ...
    default=0.5,
    show_default=True,
    help="Landmark heatmap total weight (distributed as weight/sqrt(n_landmarks)).",
)
@click.option(
    "--crop-fixed-to-overlap/--no-crop-fixed-to-overlap",
    default=True,
    show_default=True,
    help="Compute an overlap bbox for zoomed QC / optional cropped SyN domain.",
)
def main(
    workspace: Path,
    rois: tuple[str, ...],
    *,
    ...
    run_dirname: str,
    debug: bool,
    p1_threshold: float | None,
    landmark_linear_transform_type: str,
    syn_type_of_transform: str,
    syn_metric: str,
    syn_metric_param: int,
    syn_reg_iterations: str,
    syn_grad_step: float,
    syn_flow_sigma: float,
    syn_total_sigma: float,
    moving_presmooth_sigma_um: float,
    fixed_edge_guard_um: float,
    moving_edge_guard_um: float,
    use_affine_refine: bool,
    use_n4: bool,
...
# NOTE: This is expensive (apply_transforms_to_points over a dense grid). Disabled by default.
WRITE_DEFORMATION_FIELD_PNG = False
...
def run_pipeline(
    *,
    ...
    SYN_TYPE_OF_TRANSFORM = str(syn_type_of_transform)
    SYN_METRIC = str(syn_metric)
    SYN_METRIC_PARAM = int(syn_metric_param)
    SYN_REG_ITERATIONS = tuple(int(x) for x in syn_reg_iterations)
    SYN_GRAD_STEP = float(syn_grad_step)
    SYN_FLOW_SIGMA = float(syn_flow_sigma)
    SYN_TOTAL_SIGMA = float(syn_total_sigma)
    ...
    OUTDIR_NAME = str(run_dirname)
    OUTDIR = WS_OUTDIR / OUTDIR_NAME
    ...
    P1_THRESHOLD_OVERRIDE = p1_threshold
    P1_THRESHOLD = float(P1_THRESHOLD_OVERRIDE) if P1_THRESHOLD_OVERRIDE is not None else float(threshold)
        ...
        + json.dumps(
            {
                ...
                "syn_reg_iterations": list(SYN_REG_ITERATIONS),
                "syn_grad_step": SYN_GRAD_STEP,
                "syn_flow_sigma": SYN_FLOW_SIGMA,
                "syn_total_sigma": SYN_TOTAL_SIGMA,
                "moving_presmooth_sigma_um": MOVING_PRESMOOTH_SIGMA_UM,
                ...
                "landmark_heatmap_weight": LANDMARK_HEATMAP_WEIGHT,
                "landmark_heatmap_weight_scaling": "sqrt_n",
                "crop_fixed_to_overlap": CROP_FIXED_TO_OVERLAP,
                "write_deformation_field_png": WRITE_DEFORMATION_FIELD_PNG,
            },
            indent=2,
        )
    )
    ...
    QC_MOVING_BBOX_LANDMARKS_PNG = OUTDIR / 'moving_before_after_bbox_landmarks.png'
    ...
    if WRITE_DEFORMATION_FIELD_PNG:
        ...
        fig_w_in = min(
            float(DEFORMATION_MAX_FIG_IN),
            max(10.0, float(moving_bg_ds.shape[0]) / float(DEFORMATION_SAVE_DPI)),
        )
        ...
        print("Skipping deformation field PNG (disabled).")
    ...
    if union_bbox is not None:
        ...
        # Landmarks overlay (fixed landmarks vs moving landmarks mapped into fixed space)
        df_moving = pd.DataFrame(moving_pts_mm, columns=["x", "y"])
        df_moving["z"] = 0.0
        df_moving["t"] = 0.0
        if USE_AFFINE_REFINE and tx_affine_refine is not None and affine_refine_applied:
            inv_init = tx_affine_refine.get("invtransforms")
            if isinstance(inv_init, str):
                inv_init_list = [inv_init]
            elif isinstance(inv_init, (list, tuple)) and all(isinstance(p, str) for p in inv_init):
                inv_init_list = list(inv_init)
            else:
                raise TypeError(f"Unexpected affine refine invtransforms type: {type(inv_init)!r}")
            df_init_in_fixed = ants.apply_transforms_to_points(dim=2, points=df_moving, transformlist=inv_init_list)
        else:
            init_list = list(initial_transform_for_syn)
            whichtoinvert: list[bool] = []
            for path in init_list:
                ...
                if suffix == ".mat":
                    ...
                    raise ValueError(f"Cannot invert init transform {path!r} (suffix {suffix!r}). Expected '.mat'.")
            df_init_in_fixed = ants.apply_transforms_to_points(
                dim=2,
                points=df_moving,
                transformlist=init_list,
                whichtoinvert=whichtoinvert,
            )
        moving_init_in_fixed_mm = df_init_in_fixed[["x", "y"]].to_numpy(dtype=np.float64)

        inv_list = [tx["invtransforms"]] if isinstance(tx["invtransforms"], str) else list(tx["invtransforms"])
        df_final_in_fixed = ants.apply_transforms_to_points(dim=2, points=df_moving, transformlist=inv_list)
        moving_final_in_fixed_mm = df_final_in_fixed[["x", "y"]].to_numpy(dtype=np.float64)

        fixed_bbox = ants.crop_indices(fixed_ants, bb_lo, bb_hi)
        fixed_bg = normalize_robust(ants_numpy_yx(fixed_bbox).astype(np.float32))

        fig, axes = plt.subplots(1, 2, figsize=(10, 5))
        axes[0].imshow(fixed_bg, cmap="gray", interpolation="nearest")
        fx0, fy0, f0_ok = _points_mm_to_vox_xy(fixed_pts_mm, fixed_bbox)
        mx0, my0, m0_ok = _points_mm_to_vox_xy(moving_init_in_fixed_mm, fixed_bbox)
        ok0 = f0_ok & m0_ok
        axes[0].scatter(fx0[f0_ok], fy0[f0_ok], s=38, c="magenta", marker="x", linewidths=1.4, label="fixed")
        axes[0].scatter(
            mx0[m0_ok],
            my0[m0_ok],
            s=44,
            ...
            edgecolors="lime",
            linewidths=1.4,
            label="moving→fixed (init)",
        )
        for fx, fy, mx, my in zip(fx0[ok0], fy0[ok0], mx0[ok0], my0[ok0], strict=False):
            axes[0].annotate(
                "",
                xy=(float(fx), float(fy)),
                xytext=(float(mx), float(my)),
                arrowprops={"arrowstyle": "->", "color": "yellow", "lw": 1.0, "alpha": 0.85},
            )
        axes[0].set_title("Fixed + landmarks (init)")
        axes[0].legend(loc="lower right", fontsize=8, frameon=False)
        axes[0].axis("off")

        axes[1].imshow(fixed_bg, cmap="gray", interpolation="nearest")
        fx1, fy1, f1_ok = _points_mm_to_vox_xy(fixed_pts_mm, fixed_bbox)
        mx1, my1, m1_ok = _points_mm_to_vox_xy(moving_final_in_fixed_mm, fixed_bbox)
        ok1 = f1_ok & m1_ok
        axes[1].scatter(fx1[f1_ok], fy1[f1_ok], s=38, c="magenta", marker="x", linewidths=1.4, label="fixed")
        axes[1].scatter(
            mx1[m1_ok],
            my1[m1_ok],
            s=44,
            ...
            edgecolors="lime",
            linewidths=1.4,
            label="moving→fixed (SyN)",
        )
        for fx, fy, mx, my in zip(fx1[ok1], fy1[ok1], mx1[ok1], my1[ok1], strict=False):
            axes[1].annotate(
                "",
                xy=(float(fx), float(fy)),
                xytext=(float(mx), float(my)),
        ...
        axes[1].set_title("Fixed + landmarks (SyN)")
        axes[1].legend(loc="lower right", fontsize=8, frameon=False)
        axes[1].axis("off")

        fig.suptitle(
            f"Fixed bbox crop + landmarks | MI(opt,masked) {mi_before:.3f}→{mi_after:.3f} | "
            f"MI(raw) {mi_before_raw:.3f}→{mi_after_raw:.3f} | lm_rmse={lm_err['rmse_mm']*1e3:.1f} µm"
        )
        plt.tight_layout()
        plt.savefig(QC_MOVING_BBOX_LANDMARKS_PNG, dpi=int(QC_SAVE_DPI))
        plt.close(fig)
    else:
    ...
    summary = {
        ...
        "deformation_field_png": str(DEFORMATION_FIELD_PNG) if WRITE_DEFORMATION_FIELD_PNG else None,
        "write_deformation_field_png": bool(WRITE_DEFORMATION_FIELD_PNG),
        "use_landmark_heatmap_metric": USE_LANDMARK_HEATMAP_METRIC,
        ...
        "paths": {
            ...
            "qc_moving_bbox_landmarks_png": str(QC_MOVING_BBOX_LANDMARKS_PNG) if union_bbox is not None else None,
    ...
    if union_bbox is not None:
        ...
        print(f"Wrote: {QC_MOVING_BBOX_LANDMARKS_PNG}")
    if fixed_crop_lower is not None and fixed_crop_upper is not None:
