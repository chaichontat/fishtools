...
def single(
    ...
    if listfailed or listfailedall:
        for gene in genes:
            if not Path(
                codebook_path.parent / f"output/{hash_codebook(codebook)}" / f"{gene}_final_BamHIKpnI.parquet"
            ).exists():
                print(gene)
                if listfailedall:
    ...
    with ProcessPoolExecutor(8, mp_context=context) as exc:
        futs = {
            gene: exc.submit(
                run_gene,
                path,
