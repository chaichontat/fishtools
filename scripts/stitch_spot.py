...
res = (pca := PCA(n_components=2)).fit_transform(coords[["x", "y"]])


# %%
plt.scatter(res[:, 0], res[:, 1], s=5, alpha=0.9)
# %%
r = coords[["x", "y"]] @ pca.components_
plt.scatter(r[:, 0], r[:, 1], s=5, alpha=0.9)
# %%
u = pca.components_
# %%
baddies = []
...
# %%
plt.scatter(u["field_2"], u["field_1"], s=0.001, alpha=0.3)
