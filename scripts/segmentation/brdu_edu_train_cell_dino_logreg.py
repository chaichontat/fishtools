...
FEATURE_CACHE_VERSION = "v1_masked_brdu_edu_zero_pad5_uint8"
...
def _parse_args() -> TrainConfig:
    parser = argparse.ArgumentParser(
        description="Train frozen Cell-DINO ViT-S/8 logistic regressions on labeled BrdU/EdU barrage cells."
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Output joblib path. Default: <barrage-dir>/brdu_edu_cell_dino_cp_vits8_logreg.joblib",
    )
    parser.add_argument(
        "--feature-cache",
        type=Path,
        default=None,
        help="Output joblib path for cached Cell-DINO features. Default: <barrage-dir>/cell_dino_cp_vits8_feature_cache.joblib",
    )
    parser.add_argument("--dinov2-repo", type=Path, default=Path("~/dinov2").expanduser())
    parser.add_argument(
        "--weights",
        type=Path,
        default=Path("~/cell_dino_vits8_pretrain_cp-37d20e9c.pth").expanduser(),
    )
    ...
    parser.add_argument("--crop-eval-size", type=int, default=128)
    ...
    feature_cache = (
        ...
        else (barrage_dir / "cell_dino_cp_vits8_feature_cache.joblib")
    )
    return TrainConfig(
...
def _compose_cell_dino_channels(masked_thumb: np.ndarray, *, mask: np.ndarray) -> np.ndarray:
    """Map masked BrdU/EdU into uint8 and zero-pad to Cell-DINO's 5-channel input."""
...
class BarrageCellDataset(Dataset[tuple[np.ndarray, np.ndarray]]):
    ...
    def _load_masked_cell_image(self, row: pd.Series) -> np.ndarray:
        ...
        masked_thumb[~mask] = 0
        return _compose_cell_dino_channels(masked_thumb, mask=mask)
...
def _best_logreg_for_target(
    *,
    ...
    return best_model, best_c, best_accuracy
...
def _make_split_tables(train: pd.DataFrame, *, cfg: TrainConfig) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    rel_val_fraction = cfg.val_fraction / (1.0 - cfg.test_fraction)
    train_idx, val_idx = train_test_split(
        np.arange(len(train_val)),
        test_size=rel_val_fraction,
        random_state=cfg.seed,
        stratify=_choose_stratify_labels(train_val),
    )

    return (
        train_val.iloc[np.sort(train_idx)].reset_index(drop=True),
        train_val.iloc[np.sort(val_idx)].reset_index(drop=True),
        test,
    )
...
def main() -> None:
    ...
    train_rows, val_rows, test_rows = _make_split_tables(train_table, cfg=cfg)
    logger.info(f"Using labeled barrage rows: train={len(train_rows)}, val={len(val_rows)}, test={len(test_rows)}")

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    cache_bundle = _load_feature_cache(cfg)
    all_rows = pd.concat([train_rows, val_rows, test_rows], axis=0, ignore_index=True)
    ...
    model = _load_cell_dino_model(repo=cfg.dinov2_repo, weights=cfg.weights, device=device) if needs_extraction else None
    ...
    val_features, val_targets, val_cache_updated = _extract_features_with_cache(
        model=model,
        rows=val_rows,
        cfg=cfg,
        batch_size=cfg.batch_size,
    ...
    if train_cache_updated or val_cache_updated or test_cache_updated:
        _save_feature_cache(cfg, cache_bundle)

    train_scaler, train_scaled, val_scaled, test_scaled = _standardize_features(
        train_features=train_features,
        val_features=val_features,
        test_features=test_features,
    )

    _brdu_model, brdu_c, brdu_val_accuracy = _best_logreg_for_target(
        train_features=train_scaled,
        train_labels=train_targets[:, 0],
        val_features=val_scaled,
        val_labels=val_targets[:, 0],
    )
    _edu_model, edu_c, edu_val_accuracy = _best_logreg_for_target(
        train_features=train_scaled,
        train_labels=train_targets[:, 1],
        val_features=val_scaled,
        val_labels=val_targets[:, 1],
    )
    ...
    final_train_scaled = final_scaler.fit_transform(final_train_features)
    ...
    brdu_final.fit(final_train_scaled, final_train_targets[:, 0])
    edu_final = LogisticRegression(solver="lbfgs", C=edu_c, max_iter=1_000, tol=1e-12)
    edu_final.fit(final_train_scaled, final_train_targets[:, 1])
    ...
    print(f"brdu_val_accuracy={brdu_val_accuracy:.6f}")
    print(f"edu_val_accuracy={edu_val_accuracy:.6f}")
