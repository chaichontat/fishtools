...
pick_ref_auto <- function(u_eps) {
  med <- apply(u_eps, 2, median)
  if (any(!is.finite(med))) stop("Non-finite medians encountered in auto ref selection.")
  which.max(med)
}

fit_alr_gam <- function(
  z,
  r_um,
  theta,
  AP_um,
  ...
  if (ap_unique <= 1 || ml_unique <= 1) {
    # Avoid mgcv failures for constant covariates.
    smooth_terms <- paste0(
      if (!has_theta) "" else " + s(theta, bs = 'cc', k = k_theta)",
      sprintf(" + s(r_um, bs = '%s', k = k_r)", bs_r),
      if (!has_theta) "" else sprintf(" + ti(r_um, theta, bs = c('%s', 'cc'), k = k_rtheta)", bs_r)
...
if (length(args) < 1) {
  cat("Usage: Rscript scripts/gam/fit_inm_simplex_panel.R IN_DIR --usage-tsv PATH [--out-tsv PATH] [--eps X] [--alr-ref N|auto] [--r-max X] [--no-theta] [--threads N] [--bam-threads N] [--basis shrink|standard] [--k-uv N]\n")
  quit(status = 2)
}
...
parse_opts <- function(opts) {
  usage_tsv <- NULL
  out_tsv <- file.path(in_dir, "fit_results.simplex.tsv")
  eps <- 1e-4
  alr_ref <- "auto"
  r_max <- Inf
  use_theta <- TRUE
  ...
  while (i <= length(opts)) {
    ...
    if (opt == "--alr-ref") {
      if (i == length(opts)) stop("--alr-ref requires an integer topic index or 'auto'.")
      alr_ref <- as.character(opts[[i + 1L]])
      i <- i + 2L
      next
    }
    if (opt == "--r-max") {
...
if (is.null(usage_tsv) || !nzchar(usage_tsv)) stop("--usage-tsv is required (no implicit Usage_* in cells.tsv yet).")
...
if (is.finite(r_max)) {
  keep <- r_um0 <= r_max
  if (any(!is.finite(keep))) stop("Invalid r_um filter mask.")
...
ref_idx <- NA_integer_
if (identical(tolower(alr_ref_arg), "auto")) {
  ref_idx <- pick_ref_auto(u_eps)
} else {
  ref_idx <- suppressWarnings(as.integer(alr_ref_arg))
  if (!is.finite(ref_idx) || is.na(ref_idx) || ref_idx < 1 || ref_idx > ncol(u_eps)) {
    stop(sprintf("--alr-ref must be 'auto' or an integer in [1,%d]", ncol(u_eps)))
  }
}
...
z_mat <- matrix(NA_real_, nrow(u_eps), ncol(u_eps) - 1)
z_topics <- integer(ncol(z_mat))
jj <- 1
for (k in seq_len(ncol(u_eps))) {
  if (k == ref_idx) next
  z_mat[, jj] <- log(u_eps[, k] / u_eps[, ref_idx])
  z_topics[[jj]] <- topic_ids[[k]]
  jj <- jj + 1
}
if (any(!is.finite(z_mat))) stop("Non-finite ALR coordinates encountered; check eps and usage.")

out_dir <- dirname(out_tsv)
dir.create(out_dir, showWarnings = FALSE, recursive = TRUE)
fits_dir <- file.path(out_dir, "fits_rds__fit_results_simplex")
...
fit_one <- function(j) {
  comp_topic <- z_topics[[j]]
  comp_name <- sprintf("ALR_P%d_vs_P%d", comp_topic, ref_topic)
  fit <- tryCatch(
    {
      fit_alr_gam(
        z = z_mat[, j],
...
if (is.null(tab) || nrow(tab) <= 0) stop("No ALR components fit successfully.")

ref_row <- data.frame(
  gene = sprintf("REF_P%d", ref_topic),
  program = ref_topic,
  ref_program = ref_topic,
  is_ref = TRUE,
  p_spatial = NA_real_,
...
tab_all <- rbind(ref_row, tab)
write.table(tab_all, file = out_tsv, sep = "\t", row.names = FALSE, quote = FALSE)
cat(sprintf("Wrote %s (n=%d rows)\n", out_tsv, nrow(tab_all)))
...
meta_path <- file.path(out_dir, "simplex_meta.json")
