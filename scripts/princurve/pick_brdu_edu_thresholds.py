...
def pick_thresholds(
    *,
    ...
    brdu_vmin = float(np.nanmedian(y_brdu_all))
    brdu_vmax = float(np.nanpercentile(y_brdu_all, 99.9))
    edu_vmin = float(np.nanmedian(y_edu_all))
    edu_vmax = float(np.nanpercentile(y_edu_all, 99.9))
...
jobs = build_jobs(folder=FOLDER, skip_completed=SKIP_COMPLETED)
