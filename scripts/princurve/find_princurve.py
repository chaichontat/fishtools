...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        default=True,
        help=(
            "Compute flattened chart coordinates in microns and store as obsm['AP_ML_um'] "
            "(default: on; requires p1_landmarks.json + refextract LUT artifacts)."
        ),
    )
    p.add_argument(
        "--ap-ml-lut-outdir",
...
class _ApMlContext:
    ...
    s2c: S2CLut
    anchor_keys_i: np.ndarray
    anchor_t0: np.ndarray
    anchor_len: np.ndarray


_AP_ML_CONTEXT_CACHE: dict[tuple[str, int, float, int, float], _ApMlContext] = {}


def compute_ap_ml_um_from_refextract(
    *,
    lut_outdir: Path,
    axis: str,
    atlas_slice_idx: int,
    ...
    ref_t: float = 0.5,
    n_t: int = 257,
    dtw_band_frac: float = 0.15,
) -> np.ndarray:
    """Compute flattened (AP_um, ML_um) for points on one atlas section.

    Uses:
    - `ap_axis_um_from_strips.npz` for AP position along coronal slices
    - `coronal_midline_columns.csv` + DTW anchoring for per-slice ML arclength origin
    - `chart_map_sagittal_to_coronal_t2d.npz` to map sagittal→coronal (slice_i_float, t_cor)
    """
    outdir = Path(lut_outdir)
    ...
    if not (ap_npz.exists() and s2c_npz.exists() and cor_csv.exists()):
        raise FileNotFoundError(
            "Missing required refextract artifacts; need "
            f"{ap_npz.name}, {s2c_npz.name}, and {cor_csv.name} under {outdir}"
        )

    axis_norm = str(axis).strip().lower()
    if axis_norm not in {"coronal", "sagittal"}:
        raise ValueError(f"Unsupported axis={axis!r}; expected 'coronal' or 'sagittal'.")

    cache_key = (str(outdir), int(ref_slice_i), float(ref_t), int(n_t), float(dtw_band_frac))
    ctx = _AP_ML_CONTEXT_CACHE.get(cache_key)
    if ctx is None:
        ap_slice_keys, ap_um_vals = _load_ap_axis_um(ap_npz)
        s2c = _load_s2c_t2d(s2c_npz)
        coronal_paths = _load_midline_paths_csv(cor_csv, slice_label="slice_i")
        if not coronal_paths:
            raise ValueError(f"Empty coronal midline paths loaded from {cor_csv}.")
        ...
        if anchor_keys_i.size < 2:
            raise ValueError("Need at least 2 coronal slices with ML anchors.")
        ...
        ctx = _ApMlContext(
            ...
            s2c=s2c,
            anchor_keys_i=anchor_keys_i,
            anchor_t0=anchor_t0,
            anchor_len=anchor_len,
        )
        _AP_ML_CONTEXT_CACHE[cache_key] = ctx

    t = np.asarray(t_all, dtype=np.float64).reshape(-1)
    if axis_norm == "coronal":
        cor_slice_f = np.full(t.shape, float(atlas_slice_idx), dtype=np.float64)
        cor_t = t
    else:
...
def _read_axis_and_slice_from_p1(p1_path: Path) -> tuple[str, int]:
