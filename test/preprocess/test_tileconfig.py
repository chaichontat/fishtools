...
def test_from_pos():
    ...
    pixel_size_um = 0.108
    tc = TileConfiguration.from_pos(df_pos_pd, pixel_size_um=pixel_size_um)
    ...
    scaling_factor = -1 / pixel_size_um
...
def test_from_pos_respects_pixel_size_um():
    df_pos_pd = pd.DataFrame({0: [0.0, 1.0], 1: [0.0, 1.0]})

    tc_a = TileConfiguration.from_pos(df_pos_pd, pixel_size_um=0.1)
    tc_b = TileConfiguration.from_pos(df_pos_pd, pixel_size_um=0.2)

    # Larger pixel_size_um => fewer pixels per micron => smaller coordinate span in pixels.
    span_a = float(tc_a.df["x"].max() - tc_a.df["x"].min())
    span_b = float(tc_b.df["x"].max() - tc_b.df["x"].min())
    assert span_a == pytest.approx(2.0 * span_b)
