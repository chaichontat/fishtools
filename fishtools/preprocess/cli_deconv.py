...
def _get_percentiles_for_round(
    ...
    for bit in bits:
        if bit in override:
            ...
            logger.info(
                f"Using override for bit '{bit}' to {override[bit]} in round '{round_name}'."
            )
            percentile_mins.append(min_val)
            percentile_scales.append(scale_val)
            continue

        is_rna_bit = (bit.isdigit() and int(bit) <= max_rna_bit) or re.match(
            r"^b\d{3}$", bit
        )
        if not is_rna_bit:
...
def compute_range(
    ...
    available_bits = set(
        chain.from_iterable(round_name.split("_") for round_name in rounds)
    )

    try:
        user_override_dict = {
            (parts := v.split(","))[0]: (float(parts[1]), float(parts[2]))
            for v in override
        }
    except ValueError:
    ...
    for round_name in rounds:
        ...
        if not json_sidecars:
            logger.warning(
                f"Skipping {round_name}: no '.deconv.json' files found under {path}."
            )
            continue
            ...
            if (save_float32 or save_histograms) and f32_payload is not None:
                ...
                if save_histograms and hist_payload is not None:
                    ...
                    with open(hist_path, "w", newline="") as fh:
                        ...
                        counts_list = hist_payload[
                            "counts"
                        ]  # list[np.ndarray], len=bins
                        edges_list = hist_payload[
                            "edges"
                        ]  # list[np.ndarray], len=bins+1

                        for c in range(C):
                            edges = edges_list[c]  # shape (bins+1,)
                            counts = counts_list[c]  # shape (bins,)
                            # write each bin as one CSV row
                            for b in range(bins):
                ...
                if save_float32 or save_histograms:
                    ...
                    if save_histograms:
                        rng = np.random.default_rng(
                            int(f32_cpu[0, 0, 0] * 1e6) % (2**32)
                        )
                        _, C, _, _ = res.shape
                        counts_list: list[np.ndarray] = []
                        edges_list: list[np.ndarray] = []
                        # sampling strides (Z,Y,X). Change as desired.
                        SZ, SY, SX = 4, 2, 2
                        # random offsets to avoid aliasing (uniform in [0, stride-1])
                        off_z = rng.integers(SZ) if SZ > 1 else 0
                        off_y = rng.integers(SY) if SY > 1 else 0
                        off_x = rng.integers(SX) if SX > 1 else 0

                        # per-channel edges from the *sample* (robust enough for quantiles; gamma handles tails)
                        mins_ch = cp.min(
                            res[off_z::SZ, :, off_y::SY, off_x::SX], axis=(0, 2, 3)
                        )  # (C,)
                        maxs_ch = cp.max(
                            res[off_z::SZ, :, off_y::SY, off_x::SX], axis=(0, 2, 3)
                        )  # (C,)
                        ...
                        for c in range(C):
                            lo = float(mins_ch[c].get())
                            hi = float(maxs_ch[c].get())
                            if not np.isfinite(lo) or not np.isfinite(hi) or hi <= lo:
                                lo, hi = 0.0, 1.0
                            edges_gpu = cp.linspace(
                                lo, hi, histogram_bins + 1, dtype=cp.float32
                            )

                            # subsampled pixels for this channel
                            sample_c = res[off_z::SZ, c, off_y::SY, off_x::SX]
                            # histogram on the subsample
                            h_gpu, _ = cp.histogram(sample_c, bins=edges_gpu)
                            counts_list.append(h_gpu.get().astype(np.int64, copy=False))
                            edges_list.append(
                                edges_gpu.get().astype(np.float32, copy=False)
                            )

                        # store payload (include strides/offsets and sampled counts)
                        n_samp = int(
                            res[off_z::SZ, 0, off_y::SY, off_x::SX].size
                        )  # per channel
                        hist_payload = {
                            "C": C,
                            ...
                            "edges": edges_list,
                            "strides": (SZ, SY, SX),
                            "offsets": (off_z, off_y, off_x),
                            "sampled_per_channel": n_samp,
                            "tile_shape": tuple(int(x) for x in res.shape),  # (Z,C,Y,X)
                        }
...
def run(
    ...
    *,
    ...
    if debug:
        ...
        logger.add(sys.stderr, level="DEBUG", format=log_format)
        logger.add(
            "profiling_{time}.log", level="DEBUG", format=log_format, enqueue=True
        )
    ...
    *,
    ...
    for r in rounds:
        ...
        try:
            ...
            step = 6
            logger.warning(
                "Could not determine step from metadata, using default step=6."
            )
