...
def optimize(
    ...
    try:
        existing = len(Path(path / f"opt_{codebook.stem}" / "global_scale.txt").read_text().splitlines())
        logger.info(f"Found {existing} existing rounds.")
    except FileNotFoundError:
        logger.info("No existing global scale found. Starting from scratch.")
        existing = 0

    if (path / f"opt_{codebook.stem}" / "percentiles.json").exists():
        existing_perc = json.loads(
            Path(path / f"opt_{codebook.stem}" / "percentiles.json").read_text()
        ).__len__()
