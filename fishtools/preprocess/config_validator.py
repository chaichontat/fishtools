...
class ConfigValidator:
    ...
    def suggest_fixes(config_dict: dict[str, Any]) -> dict[str, list[str]]:
        ...
        # (Deprecated) hardware section suggestions kept for backward-compat
        if "hardware" in config_dict:
            ...
            if "max_memory_mb" in hw and hw["max_memory_mb"] > 500000:
                suggestions["hardware.max_memory_mb"] = [
                    ...
                    "Note: hardware.* is deprecated; use stitching.* and per-op threads",
                ]
            if "parallel_threads" in hw and hw.get("parallel_threads", 0) > 64:
                suggestions["hardware.parallel_threads"] = [
                    "Thread count seems very high",
                    "Consider using fewer threads to avoid contention",
                    "Note: hardware.* is deprecated; use stitching.parallel_threads",
                ]
...
class ConfigDocGenerator:
    ...
    def generate_markdown_docs(output_path: Path) -> None:
        ...
        with open(output_path, "w") as f:
            ...
            f.write(
                "This document provides a comprehensive reference for all configuration options in the fishtools preprocessing pipeline.\n\n"
            )
            ...
            f.write(
                "Any configuration parameter can be overridden using environment variables with the `FISHTOOLS_` prefix.\n"
            )
            ...
            f.write('650 = "/path/to/data/560to650.txt"\n')
